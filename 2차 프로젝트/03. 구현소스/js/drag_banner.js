...
banBox.forEach((ele, idx) => {
  if (idx == 0) {
    ...
    for (let i = 1; i <= 6; i++) {
      hcode += `
        <li class="list_item">
          <a href="#" class="items">
          <div class="txt_bx">
              <span class="tag">[애니플러스샵] 4주애인 콜라보 카페 OPEN!</span>
              <span class="tit">4주애인 콜라보 카페가 열렸다!</span>
          </div>
          <div class="img_bx">
              <img src="./images/banner/list_banner0${i}.png" alt="">
          </div>
          </a>
        </li>
      `;
    ...
    dFn.qsEl(ele, ".list").innerHTML = hcode;
  } /////// if //////
...
function slideFn(selEl) {
  ...
  const slide = dFn.qsEl(sldWrap, ".list");
  // 1-3.이벤트 대상: 선택요소 하위 .ct_btn
  const ct_btn = dFn.qsaEl(sldWrap, ".ct_btn");
  // 1-4.블릿박스 대상: 선택요소 하위 .paging span
  let paging = dFn.qsEl(sldWrap, ".paging_list");

  // 대상확인
  console.log("대상", ct_btn, slide, paging);
  ...
  // 블릿li 재선택할당하기 /////
  paging = dFn.qsaEl(sldWrap, ".paging_list span");
  ...
  ct_btn.forEach((ele) => dFn.addEvt(ele, "click", goSlide));
  ...
  function goSlide() {
    ...
    // 호출확인
    console.log("나야나!", this, this.classList.contains("next_btn"));

    // classList.contains(클래스명)
    // 선택요소에 해당클래스가 있으면 true

    // 1. 오른쪽 버튼 여부 알아내기
    let isRight = this.classList.contains("next_btn");

    // 2. 슬라이드 li 새로 읽기
    let eachOne = slide.querySelectorAll("li");

    // 3. 버튼분기하기 '.next_btn' 이면 오른쪽버튼
    if (isRight) {
      // 오른쪽버튼
  ...
  // 블릿순번 변경 함수 /////////////
  function chgpaging(isRight) {
    // isRight(0-왼쪽,1-오른쪽)
    // 1. 슬라이드 순번과 일치하는 블릿에 클래스 넣기
    // 대상: .paging li -> paging변수
...
function goWhere(target) {
  ...
  if (tgLeft < -pointLeft - 50) {
    ...
    // 오른쪽버튼 클릭 이벤트 발생하기!
    // 상대적으로 현재위치에서 찾음
    dFn.qsEl(target.parentElement, ".next_btn").click();
  } //// if ////
